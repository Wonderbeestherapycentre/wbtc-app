...
import { Plus, Search, Calendar, CheckCircle2, Circle, Archive } from "lucide-react";
        ...
        <>
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Search goals or children..."
                        value={searchTerm}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 dark:bg-neutral-900/50 border-b border-gray-200 dark:border-neutral-800 text-xs uppercase text-gray-500 font-medium">
                                <th className="px-6 py-4">Goal Details</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-neutral-800">
                            {filteredGoals.length > 0 ? (
                                filteredGoals.map((goal) => (
                                    <tr
                                        key={goal.id}
                                        onClick={() => role !== "PARENT" && handleEdit(goal)}
                                        className={`group hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-colors ${role !== "PARENT" ? "cursor-pointer" : ""}`}
                                    >
                                            ...
                                            <div className="space-y-1">
                                                {(() => {
                                                    try {
                                                        ...
                                                        return Array.isArray(objs) ? objs.slice(0, 2).map((o, i) => (
                                                            <div key={i} className="flex items-start gap-2 text-xs text-gray-500 dark:text-gray-400">
                                                                <div className="min-w-[3px] h-[3px] rounded-full bg-blue-500 mt-1.5" />
                                                ...
                                                {(() => {
                                                    try {
                                                        const objs = JSON.parse(goal.objectives);
                                                        return Array.isArray(objs) && objs.length > 2 ? (
                                                            <p className="text-[10px] text-gray-400 pl-3">+{objs.length - 2} more</p>
                                                        ) : null;
                                                    } catch (e) { return null; }
                                                })()}
                                            </div>
                                        ...
                                        <td className="px-6 py-4 align-top text-right">
                                            {/* Actions can be added here or row click handles it */}
