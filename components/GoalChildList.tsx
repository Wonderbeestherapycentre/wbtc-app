...
interface GoalChildListProps {
    groups: ChildGroup[];
}

export default function GoalChildList({ groups }: GoalChildListProps) {
    return (
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50/50 dark:bg-neutral-800/50 border-b border-gray-100 dark:border-neutral-800">
                            ...
                            <th className="px-3 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Goals</th>
                            ...
                            groups.map((group, index) => (
                                <tr key={group.childId} className="group hover:bg-gray-50/50 dark:hover:bg-neutral-800/30 transition-colors">
                                    ...
                                    <td className="px-3 py-4">
                                        <Link href={`/goals?child=${group.childId}`} className="text-sm font-bold text-gray-900 dark:text-white capitalize">
                                            {group.childName}
                                        </Link>
                                    </td>
                                    <td className="px-3 py-4">
                                        <span className="text-xs text-gray-500 dark:text-gray-400">
                                            {group.activeCount} active / {group.total} total
                                        </span>
                                    </td>
                                    <td className="px-3 py-4 text-right">
                                        <Link
                                            href={`/goals?child=${group.childId}`}
