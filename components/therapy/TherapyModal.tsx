...
import { createTherapy, updateTherapy } from "@/lib/actions";
...
export default function TherapyModal({ isOpen, onClose, therapy }: TherapyModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            let result;
            if (therapy) {
                result = await updateTherapy(therapy.id, formData);
            } else {
                result = await createTherapy(formData);
            }
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-neutral-800">
                    <div>
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                            {therapy ? "Edit Therapy" : "Add New Therapy"}
                        </h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {therapy ? "Update service details" : "Create a new therapy service"}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
