...
const toDateInputValue = (value: any): string => {
    ...
    // Handles both "2026-09-08" and ISO "2026-09-08T00:00:00.000Z"
...
export default function SessionNoteForm({
    ...
    const [selectedTherapyId, setSelectedTherapyId] = useState<string>(
        isEdit ? note?.therapyId ?? "" : therapistSpecialization ?? ""
    );
    const [date, setDate] = useState<string>(
        isEdit ? toDateInputValue(note?.date) : getTodayIST()
    );
    ...
    const [activities, setActivities] = useState<{ description: string; prompt: string }[]>(() => {
        if (!isEdit) return [{ description: "", prompt: "" }];
        try {
            const acts = JSON.parse(note?.activities || "[]");
            return Array.isArray(acts) && acts.length > 0 ? acts : [{ description: "", prompt: "" }];
        } catch (e) {
            return [{ description: "", prompt: "" }];
        }
    });
    const [littleWins, setLittleWins] = useState<string>(isEdit ? note?.littleWins ?? "" : "");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            ...
            if (result.message.includes("success")) {
                ...
                setErrors(fieldErrors);
                toast.error(String(Object.values(fieldErrors)[0]));
            } else {
    ...
    return (
        <div className="bg-white dark:bg-neutral-900 max-w-2xl rounded-2xl shadow-sm border border-gray-100 dark:border-neutral-800">
            <form onSubmit={handleSubmit} className="py-2 px-4 space-y-2">
                        ...
                        <div className="max-h-60 overflow-y-auto border border-gray-200 dark:border-neutral-700 rounded-xl p-3 space-y-3">
                            {availableGoals.map((goal) => {
                                ...
                                return (
                                    <div
                                        key={goal.id}
                                        className="border-b border-gray-100 dark:border-neutral-800 pb-3 last:border-b-0 last:pb-0"
                                    >
                                        <div className="space-y-1.5">
                                            {objectives.map((objective, idx) => (
                                                <label
                                                    key={idx}
                                                    className="flex items-start gap-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-800 p-2 rounded-lg transition-colors"
                                                >
                                                    <input
                                                        type="checkbox"
                            ...
                            >
                                ...
                                <div className="flex-1 space-y-2">
                                    ...
                                    {errors[`activities.${index}.description`] && (
                                        <p className="mt-1 text-[10px] text-red-500 px-1">
                                            {errors[`activities.${index}.description`]}
                                        </p>
                                    )}
                ...
                <div className="flex justify-end gap-3 pt-6 pb-4 border-t border-gray-100 dark:border-neutral-800">
                    <button
                        type="button"
